...
| R-11 | Persistent highlight overlay removed / rects format changed | `/app/backend/server.py` (`HighlightCreate.rects`), `/app/frontend/src/pages/Reader.jsx` (`captureSelectionRects`, overlay layer) | `POST /api/highlights` MUST accept `rects: Array<{x,y,w,h}>` (normalised 0..1) and `GET /api/books/{id}/highlights` MUST return them. The Reader MUST render a `[data-testid="highlight-overlay-layer"]` containing absolute-positioned `<span>`s per rect, scaled by current page bbox. New highlights MUST capture rects; old highlights without rects fall back to card-only rendering — both behaviours must remain. |
